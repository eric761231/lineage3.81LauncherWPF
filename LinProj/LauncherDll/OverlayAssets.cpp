...
// LineageTool 等從 .tbt 匯出的靜態圖：透明區填成純紅；少數資源用洋紅。
// 用寬鬆門檻吃掉 24bpp BMP 量化誤差，避免邊緣殘紅。
static bool IsSpriteChromaKey(BYTE r, BYTE g, BYTE b) {
  if (r >= 248 && g <= 12 && b <= 12)
    return true;
  if (r >= 248 && b >= 248 && g <= 12)
    return true;
...
Gdiplus::Bitmap *OverlayAssets_CloneWithSpriteColorKey(Gdiplus::Bitmap *src) {
  ...
  Gdiplus::Bitmap *dst = new Gdiplus::Bitmap((INT)w, (INT)h, PixelFormat32bppARGB);
  ...
  for (UINT y = 0; y < h; ++y) {
    BYTE *row = (BYTE *)bd.Scan0 + (SIZE_T)y * (SIZE_T)bd.Stride;
    for (UINT x = 0; x < w; ++x) {
      BYTE *px = row + x * 4; // B,G,R,A
      if (IsSpriteChromaKey(px[2], px[1], px[0])) {
...
static bool NameNeedsSpriteColorKey(const char *name) {
  if (!name)
    return false;
  // AutoPotion／之後法術圖示：item_<gfxid>.png、skill_<id>.png
  return (_strnicmp(name, "item_", 5) == 0) ||
         (_strnicmp(name, "skill_", 6) == 0);
}
...
Gdiplus::Bitmap *OverlayAssets_GetBitmap(OverlayAssetSet *set, const char *name) {
  ...
  // tbt→BMP/PNG 紅底若直接 DrawImage 會整格紅框；道具／技能圖在 cache 前先色鍵化。
  if (NameNeedsSpriteColorKey(name)) {
    Gdiplus::Bitmap *keyed = OverlayAssets_CloneWithSpriteColorKey(bmp);
    if (keyed) {
      delete bmp;
      bmp = keyed;
      NetLog("[overlay-assets] sprite color-key applied: %s", name);
    }
  }

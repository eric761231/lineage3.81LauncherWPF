...
{
    /// <summary>
    /// 產生與登入器載入流程相容的 config.dat（標頭 26 bytes 明文含隨機 AES 金鑰，其餘經 ConfigEncrypt）。
    /// </summary>
    public static class ConfigDatWriter
    {
        ...
        /// <summary>
        /// 建立加密後的 config.dat 內容。每次呼叫會產生新的隨機 16-byte 金鑰（寫入結構體 Key 欄位）。
        /// </summary>
        public static byte[] BuildEncryptedFile(LauncherConfig cfg)
        {
            ...
            // 與 CryptoService 一致：AES 只處理完整 16-byte 區塊，尾端不足一區塊者僅經 XOR。

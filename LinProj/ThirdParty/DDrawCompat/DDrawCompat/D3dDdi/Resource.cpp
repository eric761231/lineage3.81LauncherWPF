...
#undef USE_FORMAT
	...
	void Resource::updateConfig()
	{
		...
		{
			const auto repoPtr = &m_device.getRepo();
			if (!repoPtr)
			{
				// This device's surface repository was never initialized (e.g. a
				// primary surface Restore after the game window was minimized and
				// restored can rebuild the underlying D3D device outside the usual
				// DirectDraw::onCreate() -> initRepository() path). Skip the
				// MSAA/scaling enhancement below rather than dereference a null
				// repository -- same tolerance RealPrimarySurface::flush() already
				// applies around getRepo() elsewhere in this codebase.
				return;
			}
			auto& repo = *repoPtr;

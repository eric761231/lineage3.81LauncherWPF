...
namespace
{
	...
	struct UpdateWindowContext
	{
		...
		// Deadlock fix: updateWindow() used to call Gdi::Window::updatePresentationWindowPos()
		// directly while EnumWindows was still running inside updateAll()'s
		// D3dDdi::ScopedCriticalSection scope. That function ends up doing a blocking
		// Gdi::GuiThread::execute() (SendMessageTimeout to the GUI thread), which can
		// deadlock against the GUI thread if it's concurrently waiting to enter the same
		// critical section (e.g. from Overlay::ConfigWindow/SettingControl handlers).
		// Collect the (presentationWindow, owner) pairs here instead and process them in
		// updateAll() after the lock has been released.
		std::vector<std::pair<HWND, HWND>> presentationWindowsToReposition;
		DWORD processId;
	};
	...
	BOOL CALLBACK updateWindow(HWND hwnd, LPARAM lParam)
	{
		...
		if (!isLayered)
		{
			...
			if (it->second.presentationWindow)
			{
				// Deferred to after updateAll() releases D3dDdi::ScopedCriticalSection --
				// see the comment on UpdateWindowContext::presentationWindowsToReposition.
				context.presentationWindowsToReposition.emplace_back(it->second.presentationWindow, hwnd);
			}
		}
...
namespace Gdi
{
	namespace Window
	{
		...
		void updateAll()
		{
			...
			// Deadlock fix: must run after the D3dDdi::ScopedCriticalSection scope above
			// has been released -- see the comment on
			// UpdateWindowContext::presentationWindowsToReposition.
			for (const auto& [presentationWindow, owner] : context.presentationWindowsToReposition)
			{
				Gdi::Window::updatePresentationWindowPos(presentationWindow, owner);
			}

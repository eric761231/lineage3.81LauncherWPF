...
# Generate launcher update package (GZip + first-16 XOR).

. "$PSScriptRoot\XorFile.ps1"

function Get-LinRelativePathCompat {
    param(
...
function Invoke-GenerateLauncherUpdatePackage {
    param(
    ...
    $xorKey = Get-FileEncryptKeyBytes -FileEncryptKey $FileEncryptKey
    ...
    foreach ($file in $files) {
        ...
        $relativePath = Get-LinRelativePathCompat -BasePath $SourceDir -TargetPath $fullName
